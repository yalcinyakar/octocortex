...
if(typeof module!=='undefined')module.exports={UNIT,CONCEPT,ACTION,REASON,CAPABILITY,BENCHMARK_RESULTS,GENERALIZATION_RESULTS,WORLD_MODEL_RESULTS,LIFNeuron,SparseEventBus,CapabilityRouter,DistilledBackupPlanner,OnlineVectorQuantizer,SemanticAdapter,OctoSimulation};
if(typeof document!=='undefined'){$('#step').onclick=step;$('#run').onclick=toggleRun;$('#fault').onclick=togglePlanningFault;$('#reset').onclick=reset;render(simulation.snapshot());renderBenchmark();renderGeneralization();renderWorldModel()}
